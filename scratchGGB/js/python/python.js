...
Blockly.Python.workspaceToCode = function(workspace) {
  ...
  // d'abord les définitions de procédures
  for (var x = 0, block; block = blocks[x]; x++) {
	if (blocks[x].type != "procedures_definition") continue;
    ...
    if (line) {
	  if (blocks[x].type =="procedures_definition") {
		line = line.split("\n");
		line[0] = line[0].trim();
		line = line.join("\n\t");
  ...
  // ensuite le drapeau vert
  for (var x = 0, block; block = blocks[x]; x++) {
	if (blocks[x].type != "event_whenflagclicked") continue;
    var line = this.blockToCode(block);
    if (goog.isArray(line)) {
      // Value blocks return tuples of code and operator order.
      // Top-level blocks don't care about operator order.
